<div id="filament-assistant::chat-modal-container" class="flex flex-col h-full" {!! $maxHeight ? 'style="height: ' . $maxHeight . '"' : '' !!}>
    @if($showTabs)
        <div class="modal-header">
            <ul class="flex justify-between border-b border-gray-300">
                <div class="flex" >
                    @foreach($tabs as $key => $tab)
                        @if($loop->index < $maxVisibleTabs)
                            <li class="relative mr-3" style="margin-bottom: -2px" wire:key="modal-tab-{{ $key }}">
                                <button
                                    class="inline-flex items-center space-x-2 px-4 py-2 rounded-t-lg {{ $threadId === $key ? 'bg-white text-gray-800 border border-b-1 border-b-white' : 'bg-gray-100 text-gray-500' }}"
                                    wire:click="setActiveTab({{ $key }})"
                                >
                                    <span class="truncate max-w-xs" title="{{ $tab['name'] }}">{{ \Illuminate\Support\Str::limit($tab['name'], 10) }}</span>
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-gray-400 hover:text-gray-600" viewBox="0 0 20 20" fill="currentColor" wire:click.stop="removeTab({{ $key }})">
                    ...
                    @if(count($tabs) > $maxVisibleTabs)
                        <li class="relative mr-3" style="margin-bottom: -2px" wire:click.away="$set('showMoreTabs', false)">
                            <button
                                class="inline-flex items-center space-x-2 px-4 py-2 rounded-t-lg bg-gray-100 text-gray-500"
                            ...
                            @if($showMoreTabs)
                                <div class="absolute z-10 mt-2 w-60 bg-white rounded-md shadow-lg py-1">
                                    @foreach($tabs as $key => $tab)
                                        @if($loop->index >= $maxVisibleTabs)
                                            <div class="flex items-center justify-between px-4 py-2 text-sm text-gray-700" wire:key="modal-tab-hidden-{{ $key }}">
                                                <a href="#" class="flex-grow" wire:click.prevent="setActiveTab({{ $key }}, true)">
                                                    {{ $tab['name'] }}
                                                </a>
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-gray-400 hover:text-gray-600 cursor-pointer" viewBox="0 0 20 20" fill="currentColor" wire:click.prevent="removeTab({{ $key }})">
                                                    <path fill-rule="evenodd" d="M10 9.293l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 011.414-1.414L10 8.586z" clip-rule="evenodd" />
                                                </svg>
                                            </div>
                                        @endif
    ...
    <div class="flex flex-row flex-1 overflow-y-hidden" >
        @if($showComponent)
            ...
            <div class="basis-2/5 border-l p-4 pb-0">
                @if($threadId)
